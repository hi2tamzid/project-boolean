...
@section('content')
<h1 class="text-center mt-5 ">Control Panel - Team Update Registration</h1>
...
@endif -->
<form action="{{ URL::to('/admin-team-update/'.$t->id) }}" class="signup_form mt-3 mb-5" method="post">
  {{ csrf_field() }}
  <div class="row">
    <div class="col-md-8">
      <div class="input-group mb-3">
        <span class="input-group-text" id="full_name">Team name: </span>
        <input name="name" value='{{ $t->name }}' type="text" class="form-control" placeholder="Enter team name" aria-label="team_name" aria-describedby="team_name" required>
      </div>
      @if($errors->has('name'))
    ...
    @php
        $team_member_count = DB::table('team__members')->where('team_id', '=', $t->id)->count();
        $i = 0;
        @endphp
    ...
    <div class="col-md-6">
      <div class="mb-3">
        ...
        <select name="member_number" class="form-select admin_team_members" aria-label="member_number">
          <option value="1" {{ $team_member_count == "1" ? "selected" : "" }}>1</option>
          <option value="2" {{ $team_member_count == "2" ? "selected" : "" }}>2</option>
          <option value="3" {{ $team_member_count == "3" ? "selected" : "" }}>3</option>
        </select>
      </div>
